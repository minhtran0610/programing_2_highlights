...
const vector<Command> COMMANDS = {
    ...
    {"POWER", 2, false, exponentation},
    {"EXP", 2, false, exponentation},
    {"STOP", 0, true, nullptr},
    {"QUIT", 0, true, nullptr},
    {"EXIT", 0, true, nullptr},
    {"Q", 0, true, nullptr}
...
int main() {
    ...
    while ( true ) {
        ...
        for (Command command : COMMANDS) {
            if (to_upper(command_to_be_executed) == command.str) {
                if (pieces.size() != command.parameter_number+1) {
                    cout << "Error: wrong number of parameters." << endl;
                } else {
                    if (not command.exit) {
                        string first_str = pieces.at(1);
                        string second_str = pieces.at(2);

                        double first = 0;
                        double second = 0;

                        if (not string_to_double(first_str, first) or not string_to_double(second_str, second)) {
                            cout << "Error: a non-number operand." << endl;
                        } else {
                            cout << command.action(first, second) << endl;
                        }
                    } else {
                        cout << GREETING_AT_END << endl;
                        return EXIT_SUCCESS;
